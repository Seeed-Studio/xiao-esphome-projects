...
def read_yaml_file(yaml_path: str) -> str:
    """
    Read a YAML file content, excluding AUTO-SYNC anchor markers.

    This strips the source file's own anchor markers to avoid duplication
    when syncing to target wiki documents.
    """
    with open(yaml_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()

    # Filter out AUTO-SYNC anchor lines from source
    filtered_lines = [
